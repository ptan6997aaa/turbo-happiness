...
try:
    df_fact = pd.read_excel("FactPerformance.xlsx",
                            sheet_name="Sheet1")       # 考试事实表
    df_dimStu = pd.read_excel(
        "DimStudents.xlsx", sheet_name="Sheet1")         # 学生维度
    df_dimCal = pd.read_excel(
        "DimCalendar.xlsx", sheet_name="Date")           # 日期维度
    df_dimSub = pd.read_excel(
        "DimSubjects.xlsx", sheet_name="DimSubjects")    # 学科维度（NEW）
...
df_dimCal["YearQuarterConcat"] = df_dimCal["Year"].astype(
    str) + " Q" + df_dimCal["QuarterNumber"].astype(str)
...
external_stylesheets = [
    ...
    "https://cdnjs.cloudflare.com/ajax/libs/primeflex/4.0.0/themes/primeone-light.min.css",
]
...
app.layout = dbc.Container([
    ...
    # 顶部：筛选状态 + 重置按钮
    dbc.Row([
        dbc.Col([html.H5(id='filter-status-text',
                className="text-primary")], width=8),
        dbc.Col([dbc.Button("Reset All Filters", id="reset-btn",
                color="secondary", size="sm", className="w-100")], width=4)
    ], className="mb-4"),
    ...
    #                 html.H3(id="kpi-avg")])], style=card_style_purple),
    #         width=3),
    #     dbc.Col(
    #         dbc.Card([
    #             dbc.CardBody([
    #                 html.H6("Weighted Avg"),
    #                 html.H3(id="kpi-weighted")])], style=card_style_purple),
    #         width=3),
    #     dbc.Col(
    #         dbc.Card([
    #             dbc.CardBody([
    #                 html.H6("Pass Rate"),
    #                 html.H3(id="kpi-pass")])], style=card_style_purple),
    #         width=3),
    #     dbc.Col(
    #         dbc.Card([
    #             dbc.CardBody([
    #                 html.H6("Perfect Scores"),
    #                 html.H3(id="kpi-perfect")])], style=card_style_purple),
    #         width=3),
    # ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(
            dbc.Card([
                dbc.CardBody([
                    html.H6("Average Score"),
                    html.H3(id="kpi-avg", className="text-teal-800")
                ])
            ]),
            width=3
        ),
        dbc.Col(
            dbc.Card([
                dbc.CardBody([
                    html.H6("Weighted Avg"),
                    html.H3(id="kpi-weighted", className="text-orange-700")
                ])
            ]),
            width=3
        ),
        dbc.Col(
            dbc.Card([
                dbc.CardBody([
                    html.H6("Pass Rate"),
                    html.H3(id="kpi-pass", className="text-green-500")
                ])
            ]),
            width=3
        ),
        dbc.Col(
            dbc.Card([
                dbc.CardBody([
                    html.H6("Perfect Scores"),
                    html.H3(id="kpi-perfect", className="text-cyan-500")
                ])
            ]),
            width=3
        ),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([dbc.CardHeader("Grade Distribution"), dbc.CardBody([dcc.Graph(
            id="chart-grade", config={'displayModeBar': False}, style={'height': '250px'})])]), width=6),
        dbc.Col(dbc.Card([dbc.CardHeader("Grade Level Distribution"), dbc.CardBody([dcc.Graph(
            id="chart-level", config={'displayModeBar': False}, style={'height': '250px'})])]), width=6),
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardHeader([
                dbc.Row([
                    dbc.Col(html.Span(id="time-chart-title", children="Performance Over Time"),
                            width=7, className="align-self-center"),
                    dbc.Col(
                        dbc.RadioItems(
                            id="time-view-toggle",
            ...
            dbc.CardBody([dcc.Graph(id="chart-time-trend",
        ...
        dbc.Col(dbc.Card([
            ...
            dbc.CardBody([dcc.Graph(
                id="chart-subject", config={'displayModeBar': False}, style={'height': '300px'})])
        ]), width=6),
    ])
], fluid=True)
...
def update_ui(selected_grade, selected_level, selected_time, selected_subject, view_mode):
    ...
    if df_grade_ctx.empty:
        ...
        df_agg_grade = df_grade_ctx.groupby('Assessment_Grade', observed=False)[
        ...
        if selected_grade != "All":
            fig_grade.update_traces(
                pull=[0.1 if x == selected_grade else 0 for x in df_agg_grade['Assessment_Grade']])
        fig_grade.add_annotation(
            text=f"{len(fully_filtered):,}<br>Assessments", x=0.5, y=0.5, showarrow=False, font_size=16)
        fig_grade.update_layout(margin=dict(
    ...
    if df_level_ctx.empty:
        ...
        df_agg_level = df_level_ctx.groupby('GradeLevel', observed=False)[
            'StudentID'].nunique().reset_index()
        fig_level = px.pie(df_agg_level, values='StudentID',
                           names='GradeLevel', hole=0.6)
        if selected_level != "All":
            fig_level.update_traces(
                pull=[0.1 if x == selected_level else 0 for x in df_agg_level['GradeLevel']])
        fig_level.add_annotation(
            text=f"{fully_filtered['StudentID'].nunique():,}<br>Students", x=0.5, y=0.5, showarrow=False, font_size=16)
        fig_level.update_layout(margin=dict(
            t=10, b=10, l=10, r=10), showlegend=False)
    ...
    d_time = df.copy()
    if selected_grade != "All":
        d_time = d_time[d_time["Assessment_Grade"] == selected_grade]
    if selected_level != "All":
        d_time = d_time[d_time["GradeLevel"] == selected_level]
    if selected_subject != "All":
        d_time = d_time[d_time["SubjectName"] == selected_subject]
    ...
    if view_mode == "Month":
        if selected_time != "All" and 'Q' in selected_time:
            ...
            parent_q = df[df["YearMonthConcat"] ==
                          selected_time]["YearQuarterConcat"].iloc[0]
    ...
    if d_time.empty:
        ...
        df_bar_time = d_time.groupby(
            time_col)["Score"].mean().reset_index().sort_values(time_col)
        ...
        # 高亮选中项（通过 opacity 控制）
        opacities = [1.0 if (selected_time == "All") or (
            x == selected_time) else 0.3 for x in df_bar_time[time_col]]
        fig_time.update_traces(marker=dict(opacity=opacities))
        fig_time.add_hline(y=global_avg_line, line_dash="dash",
                           line_color="red", annotation_text=f"Avg: {global_avg_line:.1f}")
        fig_time.update_layout(margin=dict(
            t=20, b=20, l=20, r=20), xaxis_title=None, yaxis_title="Avg Score")
    ...
    if df_sub_ctx.empty:
        ...
        df_bar_sub = df_sub_ctx.groupby("SubjectName")["Score"].mean(
        ).reset_index().sort_values("Score", ascending=False)
        fig_subject = px.bar(df_bar_sub, x="SubjectName",
                             y="Score", text_auto='.1f')
        sub_opacities = [1.0 if (selected_subject == "All") or (
            x == selected_subject) else 0.3 for x in df_bar_sub["SubjectName"]]
        fig_subject.update_traces(marker=dict(opacity=sub_opacities))
        fig_subject.add_hline(y=global_avg_line, line_dash="dash",
                              line_color="red", annotation_text=f"Avg: {global_avg_line:.1f}")
        fig_subject.update_layout(margin=dict(
            t=20, b=20, l=20, r=20), xaxis_title=None, yaxis_title="Avg Score")
